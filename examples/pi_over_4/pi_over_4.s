...
__MAIN:             MOV.L   R7, __STACK@LSB     ; Set SP to our built-in stack
                    BR      OVER_STACK          ; Skip over the stack
__STACK:            DS      16

__STACK_TOP:
OVER_STACK:         MOV.H   R3, #0x40           ; R3 <= 0x4000 = 1.0
                    MOV.L   R4, #3              ; R4 <= 0x0003 = n
...
PI_LOOP:            MOV     R1, R4              ; R1 <= R4 (n)
                    ...
                    SRCC                        ; Clear carry
                    SUB     R3, R3, R1          ; R3 <= R3 - R1 = sum - (1 / n) (ORS clears [C]arry)
                    ADD     R1, R4, #2          ; R1 <= R4 + 2
...
NEXT_ITERATION:     ;
                    ...
                    ;
                    ; Shift the low word, pushing its bit 15 into [C]arry,
                    ; then shift that into the high word:
                    ;
                    SRCC
                    LSL     R2, R2, #1          ; R2 <= R2 << 1, bit 15 => [C]arry
                    LSL     R3, R3, #1          ; R3 <= (R3 << 1) | [C]arry
                    BR      DIV_LOOP
...
EXIT_LOOP:          ;
                    ...
                    LSL     R3, R3, #1          ; R3 <= (R3 << 1) | [C]arry
                    CMP     R2, R1              ; R2 == R1?
                    BR.CS   EXIT_FN             ; R2 < R1, skip the round-up
